...
\sjtusetup{
  ...
  info = {%
    ...
    % 标题
    %
    zh / title           = {注意力分布与子空间重要性感知的KV缓存特征维压缩方法},%注意力感知的层间自适应大模型低秩KV缓存特征维压缩,基于低秩性质的大模型KV缓存压缩
    en / title           = {Attention Distribution and Subspace Importance-Aware KV Cache Feature Compression},%Attention-Aware Low-Rank Compression of KV Cache for Large Language Models 
    %
    % 标题页标题
    %   可使用“\\”命令手动控制换行
    %
    ...
    % 关键词
    %
    zh / keywords        = {大语言模型, KV缓存, 模型压缩, 低秩近似, 特征降维, 量化},
    en / keywords        = {large language model, KV cache, model compression, low-rank approximation, dimension reduction, quantization},
    %
    % 姓名
    %
    zh / author          = {杨宇博},
...
\usepackage[hidelinks]{hyperref} %去掉引用的红框
